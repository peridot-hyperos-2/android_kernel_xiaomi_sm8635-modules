...
/*********************************awinic
 * acf*************************************/
void aw882xx_dev_monitor_hal_get_time(struct aw_device *aw_dev,
                                      uint32_t *time) {
...
static void aw_dev_reg_dump(struct aw_device *aw_dev) {
  ...
  for (i = 0; i < reg_num; i++) {
    if (aw_dev->ops.aw_check_rd_access(i)) {
      ...
      aw_dev_info(aw_dev->dev, "read: reg = 0x%02x, val = 0x%04x", i, reg_val);
    }
  }
}

char *aw882xx_dev_get_ext_dsp_prof_write(void) { return &ext_dsp_prof_write; }
...
static int aw_dev_get_icalk(struct aw_device *aw_dev, int16_t *icalk) {
  ...
  if (desc->icalkl_reg == AW_REG_NONE) {
    ...
    if (aw_dev->efuse_check == AW_EF_OR_CHECK)
      reg_icalk =
          (reg_icalk >> desc->icalk_shift) | (reg_icalkl >> desc->icalkl_shift);
    else
      reg_icalk =
          (reg_icalk >> desc->icalk_shift) & (reg_icalkl >> desc->icalkl_shift);
  }
...
static int aw_dev_get_vcalk(struct aw_device *aw_dev, int16_t *vcalk) {
  ...
  if (desc->vcalkl_reg == AW_REG_NONE) {
    ...
    if (aw_dev->efuse_check == AW_EF_OR_CHECK)
      reg_vcalk =
          (reg_vcalk >> desc->vcalk_shift) | (reg_vcalkl >> desc->vcalkl_shift);
    else
      reg_vcalk =
          (reg_vcalk >> desc->vcalk_shift) & (reg_vcalkl >> desc->vcalkl_shift);
  }
  ...
  for (i = 0; i < data_len; i += 2) {
    ...
    if (aw_dev->bstcfg_enable) {
      if (reg_addr == profctrl_desc->reg) {
        profctrl_desc->cfg_prof_mode = reg_val & (~profctrl_desc->mask);
      }

      if (reg_addr == bstctrl_desc->reg) {
        bstctrl_desc->cfg_bst_type = reg_val & (~bstctrl_desc->mask);
      }
    }
    ...
    if (reg_addr == aw_dev->volume_desc.reg) {
      read_vol =
          (reg_val & (~aw_dev->volume_desc.mask)) >> aw_dev->volume_desc.shift;
    ...
    aw_dev_dbg(aw_dev->dev, "reg=0x%04x, val = 0x%04x", (uint16_t)reg_addr,
               (uint16_t)reg_val);
...
static void aw_dev_pwd(struct aw_device *aw_dev, bool pwd) {
  ...
  if (pwd) {
    aw_dev->ops.aw_i2c_write_bits(aw_dev, pwd_desc->reg, pwd_desc->mask,
                                  pwd_desc->enable);
  } else {
    aw_dev->ops.aw_i2c_write_bits(aw_dev, pwd_desc->reg, pwd_desc->mask,
                                  pwd_desc->disable);
...
static void aw_dev_amppd(struct aw_device *aw_dev, bool amppd) {
  ...
  if (amppd) {
    aw_dev->ops.aw_i2c_write_bits(aw_dev, amppd_desc->reg, amppd_desc->mask,
                                  amppd_desc->enable);
  } else {
    aw_dev->ops.aw_i2c_write_bits(aw_dev, amppd_desc->reg, amppd_desc->mask,
                                  amppd_desc->disable);
...
void aw882xx_dev_mute(struct aw_device *aw_dev, bool mute) {
  ...
  if (mute) {
    ...
    aw_dev->ops.aw_i2c_write_bits(aw_dev, mute_desc->reg, mute_desc->mask,
                                  mute_desc->enable);
...
static void aw_dev_uls_hmute(struct aw_device *aw_dev, bool uls_hmute) {
  ...
  if (uls_hmute) {
    aw_dev->ops.aw_i2c_write_bits(aw_dev, uls_hmute_desc->reg,
                                  uls_hmute_desc->mask, uls_hmute_desc->enable);
  } else {
    aw_dev->ops.aw_i2c_write_bits(aw_dev, uls_hmute_desc->reg,
                                  uls_hmute_desc->mask,
...
static void aw_dev_set_dither(struct aw_device *aw_dev, bool dither) {
  ...
  if (dither) {
    aw_dev->ops.aw_i2c_write_bits(aw_dev, dither_desc->reg, dither_desc->mask,
                                  dither_desc->enable);
  } else {
    aw_dev->ops.aw_i2c_write_bits(aw_dev, dither_desc->reg, dither_desc->mask,
                                  dither_desc->disable);
...
int aw882xx_dev_set_intmask(struct aw_device *aw_dev, bool flag) {
  ...
  if (flag)
    ret = aw_dev->ops.aw_i2c_write(aw_dev, desc->mask_reg, desc->int_mask);
  else
    ret = aw_dev->ops.aw_i2c_write(aw_dev, desc->mask_reg, desc->mask_default);
  aw_dev_info(aw_dev->dev, "done");
  return ret;
}
...
static int aw_dev_mode1_pll_check(struct aw_device *aw_dev) {
  ...
  for (i = 0; i < AW_DEV_SYSST_CHECK_MAX; i++) {
    ...
    aw_dev_dbg(aw_dev->dev, "check pll lock fail, cnt=%d, reg_val=0x%04x", i,
               reg_val);
...
static int aw_dev_mode2_pll_check(struct aw_device *aw_dev) {
  ...
  /* change mode2 */
  aw_dev->ops.aw_i2c_write_bits(aw_dev, cco_mux_desc->reg, cco_mux_desc->mask,
...
static int aw_dev_syspll_check(struct aw_device *aw_dev) {
  ...
  ret = aw_dev_mode1_pll_check(aw_dev);
  if (ret < 0) {
    aw_dev_err(aw_dev->dev, "mode1 check iis failed try switch to mode2 check");
...
static int aw_dev_sysst_check(struct aw_device *aw_dev) {
  ...
  for (i = 0; i < AW_DEV_SYSST_CHECK_MAX; i++) {
    ...
    aw_dev_info(aw_dev->dev, "check fail, cnt=%d, reg_val=0x%04x", i, reg_val);
    usleep_range(AW_2000_US, AW_2000_US + 10);
  }
  if (ret < 0)
...
void aw882xx_dev_set_fade_vol_step(struct aw_device *aw_dev,
                                   unsigned int step) {
...
int aw882xx_dev_status(struct aw_device *aw_dev) { return aw_dev->status; }
...
int aw882xx_dev_init_cali_re(struct aw_device *aw_dev) {
  ...
  if (cali_desc->mode) {
    if (cali_desc->cali_re == AW_ERRO_CALI_VALUE) {
      ret =
          aw882xx_cali_read_re_from_nvram(&cali_desc->cali_re, aw_dev->channel);
      if (ret) {
      ...
      if (cali_desc->cali_re < aw_dev->re_min ||
        ...
        aw_dev_err(aw_dev->dev, "out range re value: %d", cali_desc->cali_re);
        cali_desc->cali_re = AW_ERRO_CALI_VALUE;
...
static void aw_dev_boost_type_set(struct aw_device *aw_dev) {
  ...
  if (aw_dev->bstcfg_enable) {
    ...
    /*force boost*/
    aw_dev->ops.aw_i2c_write_bits(aw_dev, bstctrl_desc->reg, bstctrl_desc->mask,
...
void aw_dev_i2s_enable(struct aw_device *aw_dev, bool flag) {
  ...
  if (flag) {
    if (cali_desc->mode == AW_CALI_MODE_NONE)
      aw_dev->ops.aw_i2c_write_bits(aw_dev, txen_desc->reg, txen_desc->mask,
...
static int aw_device_parse_dt(struct aw_device *aw_dev) {
  ...
  aw_dev->pll_check_disable =
      of_property_read_bool(aw_dev->dev->of_node, "pll-check-disable");
  if (aw_dev->pll_check_disable) {
