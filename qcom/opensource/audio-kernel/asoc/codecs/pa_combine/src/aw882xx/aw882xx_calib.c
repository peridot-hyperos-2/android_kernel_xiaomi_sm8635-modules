...
static int aw_cali_get_read_cali_re(int32_t *cali_re, int channel) {
  ...
  if (IS_ERR(fp)) {
    aw_pr_err("channel:%d open %s failed, error=%ld", channel, AWINIC_CALI_FILE,
              PTR_ERR(fp));
    return -EINVAL;
  }
  ...
  re_buf = NULL;
  aw_pr_info("channel:%d buf:%s int_cali_re: %d", channel, buf, int_cali_re);
...
static int aw_cali_svc_get_devs_re_range(struct aw_device *aw_dev,
  ...
  list_for_each(pos, dev_list) {
    ...
    if (local_dev->channel < num) {
      /*buf[0]:pri_l_re_min, buf[1]:pri_l_re_min, buf[2]:pri_r_re_min,
       * buf[3]:pri_r_re_min*/
      data_buf[RE_MIN_FLAG + local_dev->channel * 2] = local_dev->re_min;
      data_buf[RE_MAX_FLAG + local_dev->channel * 2] = local_dev->re_max;
      cnt++;
    } else {
...
int aw_cali_store_cali_re(struct aw_device *aw_dev, int32_t re) {
  ...
  if ((re < aw_dev->re_min) || (re > aw_dev->re_max)) {
    aw_dev_info(aw_dev->dev, "re[%d] out of range {%d, %d}", re, aw_dev->re_min,
                aw_dev->re_max);
    goto store_cali_re_failed;
  }
...
static void aw_cali_svc_set_cali_status(struct aw_device *aw_dev, int status) {
  ...
  aw_dev_info(aw_dev->dev, "cali %s", (status == 0) ? ("disable") : ("enable"));
}

int aw882xx_cali_svc_get_cali_status(void) { return g_cali_status; }
...
static int aw_cali_svc_dev_get_f0_q(struct aw_device *aw_dev) {
  ...
  aw_dev_dbg(aw_dev->dev, "cali f0 is %d, q is %d", aw_dev->cali_desc.cali_f0,
             aw_dev->cali_desc.cali_q);
  return 0;
}
...
static int aw_cali_svc_dev_cali_re(struct aw_device *aw_dev,
                                   unsigned int flag) {
  int ret;
...
static int aw_cali_svc_dev_cali_f0(struct aw_device *aw_dev,
                                   unsigned int flag) {
  int ret;
...
static int aw_cali_svc_devs_cali_f0(struct aw_device *aw_dev,
                                    unsigned int flag) {
  int ret;
  struct list_head *dev_list = NULL;
...
int aw_cali_svc_get_devs_r0(struct aw_device *aw_dev, int32_t *re_buf,
                            int num) {
  int ret, cnt = 0;
  struct list_head *dev_list = NULL;
  struct list_head *pos = NULL;
...
int aw_cali_svc_get_devs_te(struct aw_device *aw_dev, int32_t *te_buf,
                            int num) {
  int ret, cnt = 0;
  struct list_head *dev_list = NULL;
  struct list_head *pos = NULL;
...
int aw_cali_svc_get_devs_st(struct aw_device *aw_dev, int32_t *st_buf,
                            int num) {
  int ret, cnt = 0;
  struct list_head *dev_list = NULL;
  struct list_head *pos = NULL;
  ...
  list_for_each(pos, dev_list) {
    local_dev = container_of(pos, struct aw_device, list_node);
    if (local_dev->channel < num) {
      ret = aw882xx_dsp_read_st(local_dev, &st_buf[local_dev->channel << 1],
...
int aw_cali_svc_get_devs_f0(struct aw_device *aw_dev, int32_t *f0_buf,
                            int num) {
  int ret, cnt = 0;
  struct list_head *dev_list = NULL;
  struct list_head *pos = NULL;
...
static int aw_cali_svc_set_devs_re_str(struct aw_device *aw_dev,
  ...
  ret = sscanf(re_str,
               "pri_l:%d pri_r:%d sec_l:%d sec_r:%d tert_l:%d tert_r:%d "
               "quat_l:%d quat_r:%d",
...
static ssize_t cali_re_show(struct device *dev, struct device_attribute *attr,
  ...
  if (is_single_cali) {
    ...
    if (ret <= 0) {
      ...
      for (i = 0; i < ret; i++)
        len += snprintf(buf + len, PAGE_SIZE - len, "%s:%d mOhms ", ch_name[i],
                        cali_re[i]);
...
static ssize_t cali_f0_show(struct device *dev, struct device_attribute *attr,
  ...
  if (is_single_cali) {
    ...
    if (ret <= 0) {
      ...
      for (i = 0; i < ret; i++)
        len += snprintf(buf + len, PAGE_SIZE - len, "%s:%d ", ch_name[i],
                        cali_f0[i]);
      len += snprintf(buf + len, PAGE_SIZE - len, "\n");
    }
  }
...
static ssize_t re_range_show(struct device *dev, struct device_attribute *attr,
  ...
  len += snprintf(buf + len, PAGE_SIZE - len, "re_min value: [%d] mOhms\n",
                  range_buf[RE_MIN_FLAG]);
  len += snprintf(buf + len, PAGE_SIZE - len, "re_max value: [%d] mOhms\n",
...
static ssize_t aw_cali_class_f0_show(struct class *class,
  ...
  if (ret <= 0) {
    ...
    for (i = 0; i < ret; i++)
      len +=
          snprintf(buf + len, PAGE_SIZE - len, "%s:%d", ch_name[i], cali_f0[i]);
    len += snprintf(buf + len, PAGE_SIZE - len, "\n");
  }
...
static ssize_t aw_class_re_range_show(struct class *class,
  ...
  for (i = 0; i < ret; i++) {
    len += snprintf(buf + len, PAGE_SIZE - len,
                    "%s:re_min:%d mOhms re_max:%d mOhms ", ch_name[i],
...
static struct class_attribute class_attr_cali_time =
    __ATTR(cali_time, 0644, aw_cali_class_time_show, aw_cali_class_time_store);

static struct class_attribute class_attr_re25_calib = __ATTR(
    re25_calib, 0644, aw_cali_class_cali_re_show, aw_cali_class_cali_re_store);

static struct class_attribute class_attr_f0_calib = __ATTR(
    f0_calib, 0644, aw_cali_class_cali_f0_show, aw_cali_class_cali_f0_store);
...
static int aw_cali_misc_open(struct inode *inode, struct file *file) {
  ...
  if (local_dev->channel != g_dev_select) {
    local_dev = list_first_entry(dev_list, struct aw_device, list_node);
    aw_dev_dbg(local_dev->dev, "can not find dev num %d, use default",
               g_dev_select);
  }
...
static int aw_cali_misc_params_ptr(struct aw_device *aw_dev,
  ...
  if (copy_from_user(p_data, (void __user *)p_params->data, p_params->len)) {
    ret = -EFAULT;
    goto exit;
  }
...
static int aw_cali_misc_read_dsp(struct aw_device *aw_dev,
                                 aw_ioctl_msg_t *msg) {
  int ret;
...
static int aw_cali_misc_write_dsp(struct aw_device *aw_dev,
                                  aw_ioctl_msg_t *msg) {
  int ret;
  char __user *user_data = (char __user *)msg->data_buf;
  uint32_t dsp_msg_id = (uint32_t)msg->opcode_id;
...
static ssize_t aw_cali_misc_read(struct file *filp, char __user *buf,
  ...
  case CALI_STR_SHOW_RE: {
    ...
    for (i = 0; i < ret; i++)
      len += snprintf(local_buf + len, sizeof(local_buf) - len, "%s:%d mOhms ",
  ...
  case CALI_STR_SHOW_CALI_F0: {
    ...
    for (i = 0; i < ret; i++)
      len += snprintf(local_buf + len, sizeof(local_buf) - len, "%s:%d ",
                      ch_name[i], temp_data[i]);
  ...
  case CALI_STR_SHOW_ST: {
    ...
    for (i = 0; i < ret; i++) {
      len += snprintf(local_buf + len, sizeof(local_buf) - len,
                      "%s:R0 %d mOhms Te %d ", ch_name[i], temp_data[i << 1],
                      temp_data[(i << 1) + 1]);
    }
    len += snprintf(local_buf + len, sizeof(local_buf) - len, "\n");
  ...
  case CALI_STR_SHOW_F0_Q: {
    ret = aw_cali_svc_get_devs_cali_f0_q(aw_dev, temp_data, temp_data1,
                                         AW_DEV_CH_MAX);
    if (ret <= 0) {
    ...
    for (i = 0; i < ret; i++)
      len += snprintf(local_buf + len, sizeof(local_buf) - len, "%s:f0:%d q:%d",
                      ch_name[i], temp_data[i], temp_data1[i]);
...
static ssize_t aw_cali_misc_write(struct file *filp, const char __user *buf,
  ...
  switch (ret) {
  case CALI_STR_CALI_RE_F0: {
    ret = aw_cali_svc_cali_cmd(aw_dev, AW_CALI_CMD_RE_F0, is_single_cali,
                               CALI_OPS_HMUTE | CALI_OPS_NOISE);
  } break;
  case CALI_STR_CALI_RE: {
    ret = aw_cali_svc_cali_cmd(aw_dev, AW_CALI_CMD_RE, is_single_cali,
  ...
  case CALI_STR_SET_RE: {
    ...
    ret = aw_cali_svc_set_devs_re_str(
        aw_dev, kernel_buf + strlen(cali_str[CALI_STR_SET_RE]) + 1);
  } break;
  case CALI_STR_DEV_SEL: {
    ret = aw_cali_misc_switch_dev(filp, aw_dev, kernel_buf);
  } break;
  case CALI_STR_CALI_F0_Q: {
    ret = aw_cali_svc_cali_cmd(aw_dev, AW_CALI_CMD_F0_Q, is_single_cali,
                               CALI_OPS_HMUTE | CALI_OPS_NOISE);
  } break;
...
int aw_cali_parse_re_dt(struct aw_device *aw_dev) {
  ...
  ret =
      of_property_read_u32(aw_dev->dev->of_node, "aw-re-min", &aw_dev->re_min);
  if (ret < 0) {
  ...
  ret =
      of_property_read_u32(aw_dev->dev->of_node, "aw-re-max", &aw_dev->re_max);
  if (ret < 0) {
