...
#define AW882XX_DRIVER_VERSION "Dv1.14.2"
...
int aw882xx_get_dev_num(void) { return g_aw882xx_dev_cnt; }
...
int aw882xx_i2c_write(struct aw882xx *aw882xx, unsigned char reg_addr,
  ...
  while (cnt < AW_I2C_RETRIES) {
    ...
    if (ret < 0) {
      aw_dev_err(aw882xx->dev, "i2c_write cnt=%d error=%d", cnt, ret);
    } else {
      if (g_print_dbg)
  ...
  if (ret < 0) {
    aw_dev_err(aw882xx->dev,
               "retray 5 times,still error,try usleep 3s, i2c_write cnt = %d "
               "error = %d",
               cnt, ret);
    ...
    if (ret < 0) {
      aw_dev_err(aw882xx->dev,
                 "usleep 3s still ereror, i2c_write cnt = %d error= %d", cnt,
                 ret);
    }
  }
...
int aw882xx_i2c_read(struct aw882xx *aw882xx, unsigned char reg_addr,
  ...
  while (cnt < AW_I2C_RETRIES) {
    ...
    if (ret < 0) {
      aw_dev_err(aw882xx->dev, "i2c_read cnt=%d error=%d", cnt, ret);
    } else {
...
static void aw882xx_start_pa(struct aw882xx *aw882xx) {
  ...
  if (aw882xx->fw_status == AW_DEV_FW_OK) {
    ...
    for (i = 0; i < AW_START_RETRIES; i++) {
      ...
      if (ret) {
        aw_dev_err(aw882xx->dev, "start failed, cnt:%d[TF-NOSOUND][AW]", i);
        continue;
      } else {
        if (aw882xx->dc_flag)
          queue_delayed_work(aw882xx->work_queue, &aw882xx->dc_work,
...
static int aw882xx_volume_put(struct snd_kcontrol *kcontrol,
  ...
  compared_vol =
      AW_GET_MAX_VALUE(vol_desc->ctl_volume, vol_desc->monitor_volume);
...
static int aw882xx_dynamic_create_controls(struct aw882xx *aw882xx) {
  ...
  aw_componet_codec_ops.add_codec_controls(aw882xx->codec, aw882xx_dev_control,
                                           AW_KCTL_NUM);
...
static void aw882xx_request_firmware(struct work_struct *work) {
  ...
  if ((ret) || (!cont)) {
    ...
    if (aw882xx->fw_retry_cnt == AW_READ_CHIPID_RETRIES) {
      ...
      /* sleep 1s */
      msleep(1000);
      aw_dev_info(aw882xx->dev, "load [%s] try [%d]!", ACF_BIN_NAME,
                  aw882xx->fw_retry_cnt);
...
static int aw882xx_set_fade_in_time(struct snd_kcontrol *kcontrol,
  ...
  if (ucontrol->value.integer.value[0] > mc->max) {
    aw_pr_dbg("set val %ld overflow %d", ucontrol->value.integer.value[0],
              mc->max);
    return 0;
  }
...
static struct snd_kcontrol_new aw882xx_controls[] = {
    SOC_ENUM_EXT("aw882xx_rx_switch", aw882xx_snd_enum[0], aw882xx_get_rx_en,
                 aw882xx_set_rx_en),
    SOC_ENUM_EXT("aw882xx_tx_switch", aw882xx_snd_enum[0], aw882xx_get_tx_en,
                 aw882xx_set_tx_en),
...
static struct snd_kcontrol_new aw882xx_spin_control[] = {
    SOC_ENUM_EXT("aw882xx_spin_switch", aw882xx_snd_enum[1], aw882xx_get_spin,
                 aw882xx_set_spin),
};
...
static void aw882xx_add_codec_controls(struct aw882xx *aw882xx) {
  ...
  aw_componet_codec_ops.add_codec_controls(aw882xx->codec, &aw882xx_controls[0],
  ...
  if (aw882xx->aw_pa->spin_desc.aw_spin_kcontrol_st == AW_SPIN_KCONTROL_ENABLE)
    aw_componet_codec_ops.add_codec_controls(
        aw882xx->codec, aw882xx_spin_control, ARRAY_SIZE(aw882xx_spin_control));
}
...
static void aw882xx_add_widgets(struct aw882xx *aw882xx) {
  ...
  for (i = 0; i < ARRAY_SIZE(aw882xx_dapm_widgets); i++) {
    if (aw882xx->rename_flag == AW_RENAME_ENABLE) {
      if (aw_widgets[i].name) {
        ret = aw882xx_append_channel_suffix("%s_%d", &aw_widgets[i].name,
                                            aw882xx);
        if (ret) {
      ...
      if (aw_widgets[i].sname) {
        ret = aw882xx_append_channel_suffix("%s_%d", &aw_widgets[i].sname,
                                            aw882xx);
        if (ret) {
          aw_dev_err(aw882xx->dev,
      ...
      if (aw_widgets[i].name) {
        ret =
            aw882xx_append_i2c_suffix("%s_%d_%x", &aw_widgets[i].name, aw882xx);
        if (ret) {
      ...
      if (aw_widgets[i].sname) {
        ret = aw882xx_append_i2c_suffix("%s_%d_%x", &aw_widgets[i].sname,
                                        aw882xx);
        if (ret) {
  ...
  for (i = 0; i < ARRAY_SIZE(aw882xx_audio_map); i++) {
    if (aw882xx->rename_flag == AW_RENAME_ENABLE) {
      if (aw_route[i].sink) {
        ret =
            aw882xx_append_channel_suffix("%s_%d", &aw_route[i].sink, aw882xx);
        if (ret < 0) {
      ...
      if (aw_route[i].source) {
        ret = aw882xx_append_channel_suffix("%s_%d", &aw_route[i].source,
                                            aw882xx);
        if (ret < 0) {
      ...
      if (aw_route[i].sink) {
        ret = aw882xx_append_i2c_suffix("%s_%d_%x", &aw_route[i].sink, aw882xx);
        if (ret < 0) {
      ...
      if (aw_route[i].source) {
        ret =
            aw882xx_append_i2c_suffix("%s_%d_%x", &aw_route[i].source, aw882xx);
        if (ret < 0) {
...
static int aw882xx_codec_probe(aw_snd_soc_codec_t *aw_codec) {
  struct aw882xx *aw882xx = aw_componet_codec_ops.codec_get_drvdata(aw_codec);
  aw_dev_info(aw882xx->dev, "enter");
...
static int aw882xx_dai_drv_append_suffix(struct aw882xx *aw882xx,
  ...
  if ((dai_drv != NULL) && (num_dai > 0))
    for (i = 0; i < num_dai; i++) {
      if (aw882xx->rename_flag == AW_RENAME_ENABLE) {
        ret = aw882xx_append_channel_suffix("%s-%d", &dai_drv->name, aw882xx);
        if (ret < 0)
          return ret;
        ret = aw882xx_append_channel_suffix(
            "%s_%d", &dai_drv->playback.stream_name, aw882xx);
        if (ret < 0)
          return ret;
        ret = aw882xx_append_channel_suffix(
            "%s_%d", &dai_drv->capture.stream_name, aw882xx);
        if (ret < 0)
          return ret;
        dev_set_name(aw882xx->dev, "%s_%d", "aw882xx_smartpa",
                     aw882xx->aw_pa->channel);
        aw_dev_info(aw882xx->dev, "change dev_name:%s", dev_name(aw882xx->dev));
      } else {
        ret = aw882xx_append_i2c_suffix("%s-%d-%x", &dai_drv->name, aw882xx);
        if (ret < 0)
          return ret;
        ret = aw882xx_append_i2c_suffix(
            "%s_%d_%x", &dai_drv->playback.stream_name, aw882xx);
        if (ret < 0)
          return ret;
...
static int aw_componet_codec_register(struct aw882xx *aw882xx) {
  ...
  ret = aw882xx->codec_ops->register_codec(aw882xx->dev, &soc_codec_dev_aw882xx,
  ...
  if (aw882xx->rename_flag == AW_RENAME_ENABLE) {
    dev_set_name(aw882xx->dev, "%d-00%x", aw882xx->i2c->adapter->nr,
                 aw882xx->i2c->addr);
    aw_dev_info(aw882xx->dev, "reset dev_name:%s", dev_name(aw882xx->dev));
  }
...
static int aw882xx_gpio_request(struct aw882xx *aw882xx) {
  ...
  if (gpio_is_valid(aw882xx->irq_gpio)) {
    ret = devm_gpio_request_one(aw882xx->dev, aw882xx->irq_gpio, GPIOF_DIR_IN,
                                "aw882xx_int");
    if (ret) {
...
static int aw882xx_parse_dt(struct device *dev, struct aw882xx *aw882xx,
  ...
  if (ret < 0) {
    aw_dev_info(aw882xx->dev, "read sync flag failed,default phase sync off");
    sync_enable = false;
  } else {
...
static int __maybe_unused fs19xx_read_chipid(struct aw882xx *aw882xx) {
  int ret = -1;
  unsigned int cnt = 0;
  unsigned int reg_value = 0;
...
static int aw882xx_interrupt_init(struct aw882xx *aw882xx) {
  ...
  if (gpio_is_valid(aw882xx->irq_gpio)) {
    ...
    ret = devm_request_threaded_irq(aw882xx->dev,
                                    gpio_to_irq(aw882xx->irq_gpio), NULL,
                                    aw882xx_irq, irq_flags, "aw882xx", aw882xx);
    if (ret != 0) {
...
static int aw882xx_awrw_write(struct aw882xx *aw882xx, const char *buf,
  ...
  if ((data_len * 5 - 1) > str_len) {
    aw_dev_err(aw882xx->dev, "data_str_len [%d], requeset len [%d]", str_len,
               (data_len * 5 - 1));
    return -EINVAL;
  }
...
static int aw882xx_awrw_parse_buf(struct aw882xx *aw882xx, const char *buf,
  ...
  if (sscanf(buf, "0x%02x 0x%02x 0x%02x 0x%02x 0x%02x", &data[AWRW_HDR_WR_FLAG],
             &data[AWRW_HDR_ADDR_BYTES], &data[AWRW_HDR_DATA_BYTES],
...
static ssize_t awrw_show(struct device *dev, struct device_attribute *attr,
  ...
  ret =
      aw882xx_i2c_reads(aw882xx, packet->reg_addr, (char *)reg_data, data_len);
  if (ret < 0) {
    ret = -EFAULT;
    goto exit;
  }

  aw_dev_info(aw882xx->dev, "reg_addr 0x%02x, reg_num %d", packet->reg_addr,
              packet->reg_num);
...
static ssize_t dsp_re_show(struct device *dev, struct device_attribute *attr,
  ...
  if (ret) {
    len += snprintf(buf + len, PAGE_SIZE - len, "read dsp_re failed!\n");
    return len;
  }
...
static ssize_t print_dbg_show(struct device *dev, struct device_attribute *attr,
  ...
  len +=
      snprintf(buf + len, PAGE_SIZE - len, "g_print_dbg : %d\n", g_print_dbg);
...
static ssize_t algo_ver_show(struct device *dev, struct device_attribute *attr,
  ...
  if (ret < 0) {
    len += snprintf(buf + len, PAGE_SIZE - len, "read algo version failed!\n");
    return len;
  }
...
static int aw882xx_i2c_probe(struct i2c_client *i2c,
  ...
  if (ret < 0) {
#if defined(CONFIG_TARGET_PRODUCT_ZORN)
    aw_dev_info(&i2c->dev, "skipping fs19xx_read_chipid");
#else
