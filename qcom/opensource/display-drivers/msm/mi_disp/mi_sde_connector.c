...
static int mi_sde_connector_populate_mi_mode_info(struct drm_connector *conn,
  ...
  list_for_each_entry(drm_mode, &conn->modes, head) {
    ...
    rc = sde_connector_get_mode_info(&c_conn->base, drm_mode, NULL, &mode_info);
    if (rc) {
...
int mi_sde_connector_set_blob_data(struct drm_connector *conn,
  ...
  msm_property_set_blob(&c_conn->property_info, blob, SDE_KMS_INFO_DATA(info),
...
int mi_sde_connector_install_properties(struct sde_connector *c_conn) {
  ...
  if (c_conn->connector_type == DRM_MODE_CONNECTOR_DSI) {
    msm_property_install_range(&c_conn->property_info, "mi_layer_info", 0x0, 0,
                               U32_MAX, 0, CONNECTOR_PROP_MI_LAYER_INFO);

    msm_property_install_blob(&c_conn->property_info, "mi_mode_info",
                              DRM_MODE_PROP_IMMUTABLE,
                              CONNECTOR_PROP_MI_MODE_INFO);

    rc = mi_sde_connector_set_blob_data(&c_conn->base,
                                        CONNECTOR_PROP_MI_MODE_INFO);
    if (rc) {
...
int mi_sde_connector_check_layer_flags(struct drm_connector *connector) {
  ...
  if (connector->connector_type == DRM_MODE_CONNECTOR_DSI) {
    c_conn = to_sde_connector(connector);
    value = sde_connector_get_property(connector->state,
    ...
    if (flags.notificationshade_changed) {
      ...
      if ((mi_get_panel_id(display->panel->mi_cfg.mi_panel_id) ==
           N8_PANEL_PA) ||
          (mi_get_panel_id(display->panel->mi_cfg.mi_panel_id) ==
           N8_PANEL_SA) ||
          (mi_get_panel_id(display->panel->mi_cfg.mi_panel_id) ==
           N18_PANEL_PA) ||
          (mi_get_panel_id(display->panel->mi_cfg.mi_panel_id) ==
        ...
        if (!flags.notificationshade_present &&
          ...
          DISP_INFO(
              "notificationshade layer disappears and updates backlight %d\n",
              display->panel->mi_cfg.last_bl_level);
        }
      }
    }
    ...
    if (flat_mode_val != mi_cfg->flat_cfg.cur_flat_state) {
      if (flat_mode_val == FEATURE_ON) {
        if (mi_get_panel_id_by_dsi_panel(dsi_display->panel) == N3_PANEL_PA &&
          ...
          mi_dsi_update_flat_mode_on_cmd(dsi_display->panel,
      ...
      mi_disp_feature_event_notify_by_type(
          ...
          sizeof(flat_mode_val), flat_mode_val);
...
int mi_sde_connector_ip_N2(struct drm_connector *connector) {
  ...
  if (mi_cfg->last_bl_level >= 0x160 && !mi_cfg->ip_state) {
    dsi_panel_acquire_panel_lock(display->panel);
    mi_dsi_update_51_mipi_cmd(display->panel, DSI_CMD_SET_MI_IP_ON,
  ...
  } else if (mi_cfg->last_bl_level != 0 && mi_cfg->last_bl_level < 0x160 &&
             mi_cfg->ip_state) {
    dsi_panel_acquire_panel_lock(display->panel);
    mi_dsi_update_51_mipi_cmd(display->panel, DSI_CMD_SET_MI_IP_OFF,
                              mi_cfg->last_bl_level);
